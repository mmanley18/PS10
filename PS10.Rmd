...
# the red line indicates the number of nonzero beta_hats associated with the lowest MSE, >10
# the blue line indicates the number of nonzero beta_hats associated with 1SE of the lowest MSE, ~5
...
# change "dummy" variable in test data to actual log values and convert those values back to Rubles
test <- test %>%
  mutate(log_price_doc = y_hat) %>%
  mutate(price_doc = exp(log_price_doc))
...
# predicted values from the 150 observations training set
log_price_doc_hat <- predict(LASSO_fit, newx=predictor_matrix_train, s=lambda_star_1SE) %>% 
  as.vector()
hist(log_price_doc_hat)

## Check RMSLE from training data for comparison
# RMSLE for predicted on train
train %>%
  mutate(price_doc_hat = exp(log_price_doc_hat)) %>%
  summarise(RMSLE = mean((log(price_doc+1) - log(price_doc_hat+1))^2))
...
# produce predictions
y_hat <- predict.lm(lm_model, newdata=test) 
...
# predicted values from the 150 observations training set
y_hat <- predict.lm(lm_model, newdata=train)

## Check RMSLE from training data for comparison
# RMSLE for predicted on train
train %>%
  mutate(price_doc_hat = exp(y_hat)) %>%
  summarise(RMSLE = mean((log(price_doc+1) - log(price_doc_hat+1))^2))
...
--------------------- | -------------   | -------------
lm                    |  0.06424726     |  2.63413
LASSO crossvalidated  |  0.2933527      |  0.45162
